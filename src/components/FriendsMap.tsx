	...
	return (
		...
		>
			<g>
				<g>
					<path
						d="M355.529,171.52H390c-5.16-43.57-24.631-83.888-56.131-115.389C302.368,24.631,262.051,5.16,218.48,0v34.471
                        C289.18,44.768,345.232,100.82,355.529,171.52z"
					/>
					<path
						d="M171.52,34.471V0C127.95,5.16,87.632,24.631,56.131,56.131C24.631,87.632,5.16,127.949,0,171.52h34.471
                        C44.768,100.82,100.82,44.768,171.52,34.471z"
					/>
					<path
						d="M218.48,355.529V390c43.569-5.16,83.888-24.631,115.389-56.131c31.5-31.501,50.971-71.818,56.131-115.389h-34.471
                        C345.232,289.18,289.18,345.232,218.48,355.529z"
					/>
					<path
						d="M34.471,218.48H0c5.16,43.569,24.631,83.888,56.131,115.389c31.501,31.5,71.818,50.971,115.389,56.131v-34.471
					...
					<path
						d="M108.471,218.48H73.433c9.538,49.473,48.614,88.549,98.087,98.086v-35.037
					...
					<path
						d="M281.529,171.52h35.038c-9.538-49.473-48.614-88.549-98.087-98.087v35.038
                        C249.076,116.782,273.218,140.924,281.529,171.52z"
					/>
					<path
						d="M204.521,149.045V0h-19.042v149.045c-18.252,3.775-32.658,18.182-36.434,36.434H0v19.042h149.045
...
const FitBoundsButton: React.FC<{ bounds?: L.LatLngBounds | null }> = ({
	bounds,
}) => {
	const map = useMap();
...
export default function FriendsMap({
	...
	useEffect(() => {
		const geocodeFriends = async () => {
			...
			setFriendsWithCoords(
				resolvedFriends.filter((f) => f.lat !== 0 && f.lng !== 0)
			);
		};
	...
	return (
		<div className="friends-map-root">
			<div className="controls">
				...
				<div className="stats">
					<small>
						{filtered.length} / {friends.length} friends shown
					</small>
				</div>
			</div>
							...
							<Popup>
								<div style={{ minWidth: 200 }} className="contact-card">
									...
									<div style={{ marginTop: 6 }}>
										<div>
											<small>From: {friend.From}</small>
										</div>
										<div>
											<small>Address: {friend["Present Address"]}</small>
										</div>
										<div>
											<small>Profession: {friend.Profession}</small>
										</div>
									</div>
									<div
										style={{
